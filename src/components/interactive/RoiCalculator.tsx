...
import { Calculator, Clock, IndianRupee, TrendingUp, Sparkles, ArrowRight, ShieldCheck } from 'lucide-react';
import { SpotlightCard } from '@/components/ui/SpotlightCard';
import { BookNowModal } from '@/components/booking/BookNowModal';

export function RoiCalculator() {
  const [videosPerMonth, setVideosPerMonth] = useState<number>(20);
  const [hoursPerVideo, setHoursPerVideo] = useState<number>(5);
  const [hourlyRate, setHourlyRate] = useState<number>(2500);
  const [isBookModalOpen, setIsBookModalOpen] = useState<boolean>(false);

  const stats = useMemo(() => {
    const totalManualHours = videosPerMonth * hoursPerVideo;
    // NovaMint pipeline automates 85% of repetitive editing, captioning, reframing & posting
    const automatedHoursSaved = Math.round(totalManualHours * 0.85);
    const monthlyCapitalSaved = automatedHoursSaved * hourlyRate;
    const yearlyCapitalSaved = monthlyCapitalSaved * 12;
    // Estimated average NovaMint retainer
    const estimatedCost = 29999;
    const paybackDays = Math.max(2, Math.round((estimatedCost / (monthlyCapitalSaved / 30))));
  ...
  }, [videosPerMonth, hoursPerVideo, hourlyRate]);
    ...
    <div className="w-full">
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        <h3 className="text-3xl md:text-4xl font-bold font-display text-white tracking-tight">
          What Is Inefficient Video Production Costing You?
        </h3>
        <p className="text-sm md:text-base text-neutral-400 mt-2">
          Adjust the sliders below to calculate how many executive hours and capital NovaMint’s AI-enhanced workflows unlock for your brand.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-7 bg-[#08090C]/90 rounded-2xl border border-white/[0.08] p-6 md:p-8 flex flex-col justify-between">
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="font-mono font-bold text-lg text-primary-300 px-3 py-1 rounded-lg bg-primary-500/10 border border-primary-500/20">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="font-mono font-bold text-lg text-accent px-3 py-1 rounded-lg bg-accent/10 border border-accent/20">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-sm font-medium text-neutral-300 flex items-center gap-2">
                  ...
                  Your Estimated Hourly Rate / Time Value
                </label>
                <span className="font-mono font-bold text-lg text-emerald-400 px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
          ...
          <div className="mt-8 pt-6 border-t border-white/[0.08] flex items-center gap-2 text-xs text-neutral-400">
            ...
            <span>Based on audited time-tracking across 40+ creator pipelines managed by NovaMint.</span>
          </div>
        </div>
          ...
          <SpotlightCard className="p-6 md:p-8 h-full flex flex-col justify-between border-primary-500/30 bg-gradient-to-br from-[#0D0B18] via-[#08090E] to-[#040507]">
            <div>
              ...
              <div className="mt-4 pb-6 border-b border-white/[0.08]">
                ...
                <div className="text-xs text-emerald-400 font-mono mt-1">
                  ≈ ₹{stats.yearlyCapitalSaved.toLocaleString('en-IN')} annualized savings
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 py-6 border-b border-white/[0.08]">
                <div>
                  ...
                  <div className="text-[11px] text-neutral-500">85% efficiency gain</div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-[11px] text-neutral-500">Break-even threshold</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 space-y-3">
              ...
              <div className="text-center text-[11px] text-neutral-500 font-mono">
                No credit card required for strategic consultation
              </div>
            </div>
          </SpotlightCard>
        </div>
